...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('menu-home-btn').onclick = () => {
        SideMenu.close();
        // Limpar sala online se estiver em uma
        if (currentRoom) {
            cleanupRoom(currentRoom);
            currentRoom = null;
        }
        // Reset estado offline
        resetOfflineState();
    ...
    document.getElementById('screen-splash').onclick = () => showScreen('screen-mode-select');
...
function executeRevealPhase(playerIdx) {
    ...
    if (p.role === 'LAW') {
        roleName.innerText = p.isDelegado ? "DELEGADO DA LEI" : "Equipe da Lei";
        ...
        if (p.isDelegado) {
            ...
            document.getElementById('delegate-target').className = "neon-text red";
        }
    } else {
        roleName.innerText = p.isBoss ? "CHEFE DA GANGUE" : "Fora-da-Lei (Outlaw)";
        roleName.className = "neon-text red";
        roleDesc.innerText = "Você está infiltrado na cidade.";
        spiesList.classList.remove('hidden');
        document.getElementById('spies-ul').previousElementSibling.className = "neon-text red";
    ...
    db.ref('rooms/' + code + '/players/' + onlineProfile.name).once('value').then(snap => {
        ...
        db.ref('rooms/' + code + '/players').once('value').then(allSnap => {
            ...
            document.getElementById('online-role-name').innerText =
                p.isDelegado ? 'DELEGADO DA LEI' :
                p.isBoss     ? 'CHEFE DA GANGUE' :
                p.role === 'LAW' ? 'Equipe da Lei' : 'Fora-da-Lei (Outlaw)';
            ...
            if (p.role === 'LAW') {
                ...
                if (p.isDelegado) {
                    db.ref('rooms/' + code + '/delegadoTargetName').once('value').then(s => {
                        document.getElementById('online-delegate-target').innerText = s.val();
